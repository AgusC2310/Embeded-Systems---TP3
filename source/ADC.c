...
static ADCIrqFun_t irq_callback;


static uint16_t dummy;
static char ready;
void ADC_Init (bool hw_trg)
{
	...
	ADC0->CFG1 = ADC_CFG1_ADIV(0x00);	//Clock divided by 1
	ADC1->CFG1 = ADC_CFG1_ADIV(0x00);

	if(hw_trg){
		ADC0->SC2 |= ADC_SC2_ADTRG(0X01);
	}

	ADC0->SC1[0] = 0x0;		//enable ADC0
	ADC0->SC1[0] |= ADC_SC1_AIEN(0X01); 	//ENABLE INTERRUPTS
	SIM->SOPT7 = (SIM->SOPT7 & ~SIM_SOPT7_ADC0TRGSEL_MASK) | SIM_SOPT7_ADC0TRGSEL(0b0); //PDB trigger
	SIM->SOPT7 = (SIM->SOPT7 & ~SIM_SOPT7_ADC0PRETRGSEL_MASK) | SIM_SOPT7_ADC0PRETRGSEL(0b0);
	SIM->SOPT7 = (SIM->SOPT7 & ~SIM_SOPT7_ADC0ALTTRGEN_MASK) | SIM_SOPT7_ADC0ALTTRGEN(0b0);
...
void setADCIRQ(ADCIrqFun_t callback){
